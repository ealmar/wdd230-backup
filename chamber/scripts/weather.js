...
//*****today icon weather */
   const iconsrc = `https://openweathermap.org/img/wn/${data.list[0].weather[0].icon}@2x.png`; 
    ...
    weatherIconDay24.innerHTML = iconsrc;
    weatherIconDay24.setAttribute('src', iconsrc);
...
//const url = "https://api.openweathermap.org/data/2.5/forecast?lat=49.74880&lon=6.63401&units=imperial&appid=648276dc852d1411613be08976455040"
const url = "https://api.openweathermap.org/data/2.5/forecast?lat=40.6916&lon=-112.0011&exclude=hourly&units=standard&appid=648276dc852d1411613be08976455040";
async function apiFetch() {
    try {
    const response = await fetch(url);
    if (response.ok) {
